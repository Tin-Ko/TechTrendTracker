...
// Real in-process ONNX embedding via hugot (cgo: ONNX Runtime + libtokenizers).
// Built only with `-tags onnx` (Dockerfile, local run.sh). The default build uses
// embed_service_stub.go so CI/tests link without the native libraries.
