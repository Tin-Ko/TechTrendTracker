...
class Processor:
    def __init__(self, api_key: str, llm_url: str, system_prompt: str, hdfs_job_desc_dir: str, hdfs_save_dir: str) -> None:
        ...
        self.llmExtractor = Extractor(api_key=api_key, base_url=llm_url, system_prompt=system_prompt)
        self.requirementsParser = RequirementsParser(canonical_skill_map=canonical_skill_map, tech_capitalization_map=tech_capitalization_map)
        self.hdfsClient = HDFSClient()
        self.job_paths = [os.path.join(hdfs_job_desc_dir, job) for job in self.hdfsClient.list_dir(hdfs_job_desc_dir)]

    def process_job_description(self, job_description: str = "") -> Dict:
        extracted_data = self.llmExtractor.extract_skills_from_job(job_description)
        cleaned_data = self.requirementsParser.clean_extracted_data(extracted_data)
    ...
    def process_job(self, job_path: str):
        ...
        job_data = self.hdfsClient.read_json(job_path)
        ...
        self.hdfsClient.write(hdfs_path=os.path.join(self.hdfs_save_dir, os.path.basename(job_path)), data=json.dumps(processed_job))
...
if __name__ == "__main__":
    ...
    processor = Processor(api_key=LLM_API_KEY, llm_url="https://api.deepseek.com", system_prompt=system_prompt, hdfs_job_desc_dir="/jobs/2025-04-17", hdfs_save_dir="/skills/2025-04-17")
